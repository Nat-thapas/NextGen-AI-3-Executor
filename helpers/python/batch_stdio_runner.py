...
def get_colored_diagnostics() -> bool:
...
def main() -> None:
    colored_diagnostics = get_colored_diagnostics()
    ...
    with open(submission_path, "r", encoding="utf-8") as submission_file:
        submission = submission_file.read()
        try:
            submission = compile(submission, "submission.py", "exec")
        except Exception:
            pass
    ...
    for idx, case in enumerate(cases):
        ...
        if pid == 0:  # Child process
            ...
            os.close(0)
            if os.open(case["input"], os.O_RDONLY) != 0:
            ...
            try:
                ...
                if sys.version_info >= (3, 13):
                    traceback.print_exception(exp, colorize=colored_diagnostics)  # type: ignore  // The type checker doesn't understand the 'colorize' kwarg for some reason
                else:
